...
def articProtocol(run_name,fast5,mini,maxi,accuracy,num_callers,gpu_runners_per_device,chunks_per_runner,numGpus,email='No'):
    ...
    def basecaller(fast5,run_name,accuracy,num_callers=8,gpu_runners_per_device=64,chunks_per_runner=256,numGpus=1):
        if accuracy == 'high':
            ...
            print("\nno es un argumento valido para elmodo Basecalling, por favor elija entre high o fast\n\n")
            exit()
        if numGpus == 1:
...
a correr el script.\n\n".format(run_name))
    ...
    if ambiente[-1] == 'artic-ncov2019':
        #Correr basecaller
        basecaller(fast5,run_name,'high',numGpus=numGpus)

        #Correr barcode
        barcodSub = config.barcoder + " --require_barcodes_both_ends -i " + run_name + " -s output --arrangements_files \"barcode_arrs_nb12.cfg barcode_arrs_nb24.cfg\""
...
' --directory output/' + filename + ' --prefix ' + run_name
        ...
        for barnames in os.listdir('./'):
            if barnames.startswith(Barcodes): 
                ...
                subArticMin = 'artic minion --normalise 200 --threads 4 --scheme-directory ~/artic-ncov2019/primer_schemes --read-file ' +\
barnames + ' --fast5-directory ' + fast5 + ' --sequencing-summary ' + sec_summ + ' nCoV-2019/V3 ' + samplename
